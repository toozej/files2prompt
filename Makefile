...
verify: ## Verify Docker image with Cosign
	cosign verify \
		--certificate-identity-regexp '^https://github.com/toozej/files2prompt/.github/workflows/release.yaml@refs/tags/.*$$' \
...
distroless-run: ## Run built Docker image using distroless as final base
	docker run --rm --name files2prompt -v $(CURDIR)/config:/config $(IMAGE_AUTHOR)/$(IMAGE_NAME):$(IMAGE_TAG)-distroless
...
local-sign: local-test ## Sign locally installed golang toolchain and cosign
	if test -e $(CURDIR)/files2prompt.key && test -e $(CURDIR)/.env; then \
		export `cat $(CURDIR)/.env | xargs` && cosign sign-blob --key=$(CURDIR)/files2prompt.key --bundle=$(CURDIR)/files2prompt.bundle $(CURDIR)/out/files2prompt; \
	else \
		echo "no cosign private key found at $(CURDIR)/files2prompt.key. Cannot release."; \
	fi
...
local-verify: get-cosign-pub-key ## Verify locally compiled binary
	...
	cosign verify-blob --key $(CURDIR)/files2prompt.pub --bundle $(CURDIR)/files2prompt.bundle $(CURDIR)/out/files2prompt
